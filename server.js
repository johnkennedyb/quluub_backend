...
const { initSqlPool, getPool } = require('./config/sql');
...
const enableSql = process.env.SQL_ENABLED === 'true' || process.env.SQL_REQUIRED === 'true';
if (enableSql) {
  initSqlPool().catch((err) => {
    console.error('❌ SQL pool initialization failed:', err?.message || err);
    if (process.env.SQL_REQUIRED === 'true') process.exit(1);
  });
}
// Keep Mongo connected unless SQL is strictly required
...
app.post('/api/payments/webhook', express.raw({ type: 'application/json' }), chooseWebhook(paymentSql.handleStripeWebhook, paymentMongo.handleStripeWebhook));
// Paystack webhook must also use raw body for HMAC verification
app.post('/api/payments/paystack-webhook', express.raw({ type: 'application/json' }), chooseWebhook(paymentSql.handlePaystackWebhook, paymentMongo.handlePaystackWebhook));
...
io.use(async (socket, next) => {
  try {
    ...
    try {
      ...
      let user = null;
      try { user = await userRepo.findById(decoded.id); } catch (_) {}
      if (!user && process.env.SQL_REQUIRED !== 'true') {
      ...
      socket.userId = (user._id || user.id || decoded.id).toString();
      socket.user = user;
      console.log('✅ User authenticated:', socket.userId, user.fname);
      next();
    } catch (jwtError) {
...
io.on('connection', (socket) => {
  ...
  socket.on('join', async (userId) => {
    ...
    try {
      await userRepo.updateById(userIdStr, { 
        lastSeen: new Date(),
      });
...
app.post('/api/user/set-online', protect, async (req, res) => {
  try {
    ...
    if (enableSql) {
      await userRepo.updateById(userId, { lastSeen: new Date() });
    } else {
      const User = require('./models/User');
...
app.get('/api/users/online', async (req, res) => {
  try {
    ...
    if (process.env.SQL_REQUIRED === 'true') {
      recentlyActiveUsers = await userRepo.listRecentlyActive(fiveMinutesAgo);
    } else {
      const UserModel = require('./models/User');
      recentlyActiveUsers = await UserModel.find({ lastSeen: { $gte: fiveMinutesAgo } }).select('_id fname lname username lastSeen isOnline');
    }
...
app.get('/api/debug/online-users', async (req, res) => {
  ...
  try {
    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
    if (process.env.SQL_REQUIRED === 'true') {
      const dbUsers = await userRepo.listRecentlyActive(fiveMinutesAgo);
      recentlyActiveUsers = dbUsers;
      console.log('💾 Recently active users from SQL:', dbUsers.map(u => ({ id: u._id, name: `${u.fname} ${u.lname}`, lastSeen: u.lastSeen })));
    } else {
      const dbUsers = await User.find({ lastSeen: { $gte: fiveMinutesAgo } }).select('_id fname lname username lastSeen isOnline');
      recentlyActiveUsers = dbUsers;
